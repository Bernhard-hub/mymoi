...
// ============================================
// TWILIO VOICE WEBHOOK - MOI Voice Assistant
// ============================================
// Flow: Anruf → Begrüßung → Aufnahme → voice-conversation (Loop)
// Alle Anrufer willkommen!
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`📞 Anruf von ${from} (${callSid})`)

    const userId = Math.abs(from.split('').reduce((a, c) => a + c.charCodeAt(0), 0))

    // Personalisierte Begrüßung
    let greeting = 'Hallo! Ich bin MOI, dein persönlicher Assistent. Was kann ich für dich tun?'
    ...
    response.say({ language: 'de-DE', voice: 'Polly.Vicki' },
      'Keine Aufnahme erkannt. Ruf gerne nochmal an!'
    )
    response.hangup()
    ...
    const response = new VoiceResponse()
    response.say({ language: 'de-DE' }, 'Ein Fehler ist aufgetreten. Bitte versuche es später nochmal.')
    response.hangup()
